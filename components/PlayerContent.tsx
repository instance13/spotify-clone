...
const PlayerContent: React.FC<PlayerContentProps> = ({ song, songUrl }) => {
  ...
  const onPlayNext = () => {
    ...
    if (!nextSong) {
      return player.setId(player.ids[0]); // back to the first song
    };
  ...
  const onPlayPrevious = () => {
    ...
    if (!previousSong) {
      return player.setId(player.ids[player.ids.length - 1]); // load the last song
    };
  ...
  const [play, { pause, sound }] = useSound(
    songUrl, // cannot change dynamically, it re-renders a new object
    {
      volume: volume,
